...
def wait_for_mva_match(driver, mva, timeout=30):
    ...
    WebDriverWait(driver, timeout).until(
        EC.text_to_be_present_in_element(
            (By.CLASS_NAME, "fleet-operations-pwa__vehicle-property-value__nwt5x3"),
            mva
        )
    )
...
def click_add_new_complaint_button(driver, timeout=15):
    ...
    try:
        Btn = WebDriverWait(driver, timeout).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(., 'Add New Complaint')]"))
        )
...
def select_pm_complaint(driver, timeout=30):
    ...
    try:
        WebDriverWait(driver, timeout).until(
            EC.presence_of_all_elements_located((By.CLASS_NAME, "fleet-operations-pwa__complaintItem__153vo4c"))
        )
    except TimeoutException:
        print("[DEBUG] No complaint tiles found within timeout.")
        return False

    complaint_tiles = driver.find_elements(By.CLASS_NAME, "fleet-operations-pwa__complaintItem__153vo4c")
    for tile in complaint_tiles:
        try:
            content = tile.find_element(By.CLASS_NAME, "fleet-operations-pwa__tileContent__153vo4c").text
            if "PM" in content:
                tile.click()
                print(f"Selected complaint with type: {content}")
                return True
        except Exception as e:
            print(f"[DEBUG] Skipped a tile due to error: {e}")
